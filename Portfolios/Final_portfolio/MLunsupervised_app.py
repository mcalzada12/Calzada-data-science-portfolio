...
with tab_model:
    ...
    if "labels" in st.session_state or "embedding" in st.session_state:
        ...
        st.markdown("---")
        st.markdown("### 📊 Results")
        with st.expander("Result Explanations"):st.markdown("""
**Silhouette Score:** Measures how similar an object is to its own cluster compared to other clusters. Range: [-1, 1].

1. Higher is better. Values near +1 indicate that the sample is far from neighboring clusters.
2. Values near 0 indicate the sample is on or very close to the boundary between two clusters.
3. Negative values indicate the sample may have been assigned to the wrong cluster.
...
2. Sensitive to the number of clusters — can be used to help choose k.

**Davies-Bouldin Index:** Average similarity between each cluster and its most similar one, where similarity is the ratio of within-cluster distances to between-cluster distances. Lower is better.

1. Lower values indicate better clustering (well-separated, compact clusters).
2. Values > 1 often indicate overlapping clusters; values close to 0 indicate very distinct clusters.
""")
